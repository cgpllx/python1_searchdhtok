...
def random_hash_info():

    return os.urandom(__info_hash__/8)
...
def encode_nodes(nodes):
    ...
    for node in nodes:
        ...
        #n.extend([node[0], dottedQuadToNum(node[1].LANip), node[1].port])
...
def unpack_hostport(addr):
    if len(addr) == 6:
        host = addr[:4]
        port = addr[4:6]
        return (unpack_host(host.encode('hex')), unpack_port(port))
    if len(addr) == 18:
        host = addr[:16]
        port = addr[16:18]
        return (unpack_host(host.encode('hex')), unpack_port(port))
