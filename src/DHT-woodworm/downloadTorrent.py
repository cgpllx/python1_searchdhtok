...
def save(filename, content):
    try:
        file = open(filename, 'wb')#打开文件 wb读写方式
        file.write(content)
        file.close()
    except IOError, e:
        print e
...
def getTorrents(info_hash):
    ...
    # print url
    try:
        torrent = urllib2.urlopen(url, timeout=30)#打开这个地址，超时30s
        buffer = BytesIO(torrent.read())#读取内容
...
if __name__ == "__main__":
    ...
    info_hash="F86E777A1FBA81AFEA6B1308E0825022AB8A3805".upper()
    print info_hash
